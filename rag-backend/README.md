...
| `GOVERNANCE_API_URL` | Governed write API base for persisting ingest proposals | `http://127.0.0.1:2393/api/governance` (the container loopback default cannot reach the governance service in another container — on the shared Docker network use the service name, e.g. `http://algerknown-web:2393/api/governance`) |
| `GOVERNANCE_PROCESSOR_SECRET` | Processor bearer secret for proposal/operation submission (must match `algerknown-web`'s `GOVERNANCE_PROCESSOR_SECRET`) | Unset (when absent, generated ingest candidates are **skipped**, not persisted) |
